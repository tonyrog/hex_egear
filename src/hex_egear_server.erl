...
%%--------------------------------------------------------------------
init([]) ->
    Joined = hex:auto_join(hex_egear),
...
handle_call({add_event,Pid,Flags,Signal,Cb}, _From, State) ->
    case egear_server:subscribe(Flags) of
	{ok,TRef} ->
...
handle_call({del_event,Ref}, _From, State) ->
	...
	{value,Sub,Subs} ->
	    ...
	    Res = egear_server:unsubscribe(Sub#sub.tref),
...
%% @end
%%--------------------------------------------------------------------
handle_info({egear_event,TRef,EventData}, State) ->
    lager:debug("egear_event: ref=~p, event=~p", [TRef, EventData]),
...
handle_info({'DOWN',Ref,process,_Pid,_Reason}, State) ->
	...
	false ->
	    {noeply,State};
	{value,Sub,Subs} ->
	    egear_server:unsubscribe(Sub#sub.tref),
